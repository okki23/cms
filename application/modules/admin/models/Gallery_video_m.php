...
class Gallery_video_m extends MY_Model {

    protected $_table_name = 'gallery_video';
    ...
    public function save($data, $id = NULL) {
        ...
        if ($id === NULL || $id === 0 || $id === "") {
            !isset($data[$this->_primary_key]) || $data[$this->_primary_key] = NULL;
             ...
             $list = array('id'=>NULL,'caption'=>$data['caption'],'video_ff'=>$data['video_ff'],'video_url'=>$data['video_url'],'embed_code'=>$data['embed_code'],'order'=>$data['order'],'modified'=>$now);
             $this->db->set($list);
            ...
            $id = $filter($id);
            $list = array('caption'=>$data['caption'],'video_ff'=>$data['video_ff'],'video_url'=>$data['video_url'],'embed_code'=>$data['embed_code'],'order'=>$data['order'],'modified'=>$now);
    ...
    public function get_new() {
        $gallery_video = new StdClass();
        $gallery_video->id = ''; 
        $gallery_video->caption = '';
        $gallery_video->video_url = '';
        $gallery_video->video_ff = '';
        $gallery_video->embed_code = '';
